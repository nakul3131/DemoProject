...
$(document).ready(function () {
    ...
    $('#credit-businessOffice-id').change(function () {
        debugger;
        var t = $(this).val();
        $.get('/TransactionChildAction/GetAuthorizedGeneralLedger', { _businessOfficeId: $(this).val(), personId: PersonId, async: false }, function (data) {
            ...
            let len = $('#credit-general-ledger-id > option').not(':first').length;
            if (len == 1) {
            ...
            //else{
            //    $("#credit-general-ledger-id").prop('selectedIndex', 0);
            //}
        });
    ...
    $('#credit-general-ledger-id').change(function () {
        ...
        $.get('/TransactionChildAction/GetCustomerWithJointAccount', { _generalLedgerId: $(this).val(), _personId: PersonId, async: false }, function (data) {
            debugger;
            let creditarray = [];
            ...
            let len = $('#credit-customer-account-id > option').not(':first').length;
            if (len == 1) {
                $(this).next("div.error").remove();
                $('#credit-customer-account-id').prop('selectedIndex', 1);
    ...
    $('#credit-customer-account-id').change(function () {
        debugger;
        let count = 0;
        let _customerAccountId = $(this).val();
        let _balanceDate = $('#TransactionDate').val();

        if (_customerAccountId != "0") {
            if (typeof _balanceDate != 'undefined' && _customerAccountId != "") {

                $.get('/TransactionChildAction/GetClosingBalance', { "_balanceDate": _balanceDate, "_customerAccountId": _customerAccountId, async: false }, function (data) {
        ...
        $.get('/TransactionChildAction/GetSharesCapitalSettingValues', { '_customerAccountId': _customerAccountId, async: false }, function (data) {
            debugger;
            ...
            if (data.IsTransactionExists == true) {
                $('#admissionfeesformembership').removeClass('d-none');
                $('#credit-number-of-shares').val(data.TotalNumberOfShares);
                $('#credit-number-of-shares').attr('min', data.TotalNumberOfShares);
                //$('#credit-modal').find('#fund-amount').html('');
            }
            else {
    ...
    $('#debit-businessOffice-id').change(function () {
        ...
        $.get('/TransactionChildAction/GetAuthorizedGeneralLedger', { _businessOfficeId: $(this).val(), personId: PersonId, async: false }, function (data) {
            ...
            $('#debit-general-ledger-id').html(items);
            let len = $('#debit-general-ledger-id > option').not(':first').length;
            if (len == 1) {
                $('#debit-general-ledger-id').prop('selectedIndex', 1);
    ...
    $('#debit-general-ledger-id').change(function () {
        debugger;
        let _balanceDate = $('#TransactionDate').val();
        $.get('/TransactionChildAction/GetCustomerWithJointAccount', { _generalLedgerId: $(this).val(), _personId: PersonId, async: false }, function (data) {
            let items = '<option value="0">Select Customer Account</option>';
            $.each(data, function (i, CustomerWithJointAccount) {
                items += "<option value='" + CustomerWithJointAccount.Value + "'>" + CustomerWithJointAccount.Text + "</option>";
            });
            $('#debit-customer-account-id').html(items);
            let len = $('#debit-customer-account-id > option').not(':first').length;
            if (len == 1) {
                $('#debit-customer-account-id').prop('selectedIndex', 1);
                $('#debit-customer-account-id').change();
            }
        });
        if (typeof _balanceDate != 'undefined' && $(this).val() != "") {

            $.get('/TransactionChildAction/GetLedgerBalance', { "_balanceDate": _balanceDate, _generalLedgerId: $(this).val(), async: false }, function (data) {

                $('.amount-badge-text1').removeClass('d-none');
                let text = "";
                if (data > 0) {
                    $(".amount-badge-text1").html('');

                    text += '<span class="badge badge-pill badge-info float-right text-white badge-text" id="badge-amount1" data-amount="' + data + '"><i class="fa fa-inr text-white fa-1x mr-1" aria-hidden="true"></i>' + data + '</span>';
                    $(".amount-badge-text1").append(text);
                }
                else {
                    $(".amount-badge-text1").html('');
            ...
            $('.badge-text-new').addClass('d-none');
            $('#badge-amount1').hide();
            $('#badge-amount1').attr('data-amount', '');
            ///$('.badge-text').hide();
        }
    ...
    $('#debit-customer-account-id').change(function (event) {
        ...
        $.get('/TransactionChildAction/IsAllowSharesWithdrawal', { '_customerAccountId': _customerAccountId, async: false }, function (data) {
            if (data == true) {
                $('#Shares_Cession_Reason').removeClass('d-none');
        ...
        $.get('/TransactionChildAction/IsAllowedPartialSharesWithdrawal', { async: false }, function (data) {
            ...
            if (data == true) {
                ...
                $('.CessionReason-oth').removeClass('d-none');
            }
            else {
        ...
        $.get('/TransactionChildAction/GetDepositeSettingsValues', { '_customerAccountId': _customerAccountId, async: false }, function (data) {
            debugger;
            if (data.GetTransactionType === 'SCP') {
                $('.shares-capital-debit').removeClass('d-none');
            }
            if (data.GetTransactionType === 'DMN') {
                $('.shares-capital-debit').removeClass('d-none');
                //$('.shares-capital-debit').addClass('d-none');
                debugger
                let text = "";
                let html = "";
                debugger;
                $.get('/TransactionChildAction/GetSharesCessationTransactionByCustomerAccountId', { _customerAccountId: _customerAccountId, async: false }, function (data) {
                    debugger;
                    ...
                    if (data.customerAccountAge >= data.requiredAgeForWithdrawal) {
                        $('#debit-general-ledger-id').focus();
                    }
                    else {

                        html += '<div class="modal-dialog modal-dialog-top modal-dialog-zoom modal-lg"><div class="modal-content"> <div class="modal-header bg-primary"><h6 class="modal-title font-weight-bold text-white justify-content-center "><img src="" width="40" height="40" />  &nbsp;&nbsp;&nbsp;<u class="mt-5">Under Maharashtra Cooperative Society Act 1960, Section 29. (A) </span></u></h6></div><div class="modal-body"> A member shall not transfer any share held by him or his interest in the capital or property of any society which not less than one year.<p class="mt-3 font-weight-bold text-primary">Are You Want To Continue By Using Power Of Section 29 (3)?</p></div><div class="modal-footer justify-content-center"><button type="button" class="btn btn-secondary  btn-Continue">Continue</button><button type="button" class="btn btn-primary btn-no">No</button></div></div></div>';
                        $('#myModal').html(html);
                        ...
                        let admissionfeesformembership = parseFloat($('#debit-admission-fees-for-member-ship').val());
                        ...
                        if (totals <= sharesholdinglimit) {
                            let value = ({ totals, admissionfeesformembership });
                            arr.push(totals);
                            let total = 0;
                            $.each(arr, function (i, val) {
                                total += parseFloat(val);
                            })
                            $('#debit-transaction-amount').val(total.toFixed(2));
            ...
            if (data.IsTransactionExists == true) {
                $('#debitadmissionfeesformembership').removeClass('d-none');
                $('#debit-number-of-shares').val(data.NumberOfShares);
                //$('#credit-number-of-shares').prop('min', data.NumberOfShares);
            }
            else {
                $('#debitadmissionfeesformembership').addClass('d-none');
            }
    ...
    $('#debit-transaction-amount').change(function (event) {
        debugger;
        if ($(this).val() != "") {
            if (schemeClosingChargesViewModel != null) {
    ...
    $.get('/TransactionChildAction/GetPersonAutoCompleteList', { async: false }, function (data) {
        ...
        $('#credit-person').autocomplete({
            source: function (request, response) {
                ...
                if (data.length > 0) {
                    ...
                    let resp = $.map(data, function (item, i) {
                        ...
                        if (fullname.toUpperCase().indexOf(request.term.toUpperCase()) != -1 || (arry[3].toUpperCase().indexOf(request.term.toUpperCase()) != -1) || (arry[5].toUpperCase().indexOf(request.term.toUpperCase()) != -1)) {
            ...
            //       var noResult = { value:"-1",label:"No results found" };
            ...
            source: function (request, response) {
                ...
                if (data.length > 0) {
                    let resp = $.map(data, function (item, i) {
                        ...
                        let trannumber3 = arry1[0];
                        let trannumber4 = arry1[1];
                        if (fullname.toUpperCase().indexOf(request.term.toUpperCase()) != -1 || (arry[4].toUpperCase().indexOf(request.term.toUpperCase()) != -1) || (arry[5].toUpperCase().indexOf(request.term.toUpperCase()) != -1)) {
    ...
    $('#credit-person-id11').autocomplete({
        source: function (request, response) {
            ...
            $.post('/TransactionChildAction/GetPersonAutoCompleteList', { _inputString: request.term, async: false }, function (data) {
                let result = [];
                let arry1 = [];
                if (data.length > 0) {
                    let resp = $.map(data, function (item, i) {
                        //let arry = item.split(',');
                        //let arry = item.split('-');
                        //let NameOfCustomerType = arry[3];
                        //let TransctionNumber;
                        //arry.push(TransctionNumber);
                        //let TransctionNumber1 = arry[11];
                        //let NameOfJointAccountHolderType = arry[10];
                        //let fullname =arry[0] + " --> " + arry[1]+ '<li>'+ arry[10] +'</li>'
                        //let fullname = arry[0] + " --> " + arry[1];
                        //let PersonId = arry[6] + "-" + arry[7] + "-" + arry[8] + "-" + arry[9] + "-" + arry[10];

                        //let fullname = item.FullName + "-->" + item.ByHand;
        ...
        select: function (event, ui, item) {
            ...
            // let arry = item.split('-');
            $('#credit-person-id2').val(ui.item.label);
            $('#credit-businessOffice-id').change()
            // $('#person-id-debit').val(ui.item.label);

            //$('#credit-person-id2').val('Banana').data('autocomplete')._trigger('select');
            //$("#badge-text").html(ui.item.TransctionNumber1);
            PersonId = ui.item.value;
            PersonText = ui.item.label;
            //$('#credit-person-id2').val(PersonText).data('autocomplete')._trigger('select');

            //$('#person-id-debit').val(ui.item.label).data('autocomplete');
    ...
    $('.checkbox_check').click(function () {
        ...
        if (CessionReason === "Other" || CessionReason != "") {
            $('.age-proof-submission').next("div.error").remove();
    ...
    //$("#by-hand").keypress(function (e) {
    //    //Regex for Valid Characters i.e. Alphabets.
    //    var regex = /^[A-Za-z]+$/;

    //    //Validate TextBox value against the Regex.
    //    var isValid = regex.test(String.fromCharCode(e.keyCode || e.which));
    //    if (!isValid) {
    //        $('#by-hand').next("div.error").remove();
    //        $('#by-hand').after('<div class="error" style="color:red">Only Alphabets allowed</div>');
    //    }
    //    else {
    //        $('#by-hand').next("div.error").remove();
    //    }
    ...
    $('#btn-edit-credit-dt').click(function () {
        ...
        if (isChecked) {
            ...
            $('#credit-1', myModal).val(columnValues[29]);
            $('#credit-2', myModal).val(columnValues[28]);
            $('#credit-3', myModal).val(columnValues[30]);
            ...
            $('#credit-transaction-amount', myModal).val(columnValues[15]);
            $('#credit-start-certificate-number', myModal).val(columnValues[19]);
            $('#credit-end-certificate-number', myModal).val(columnValues[20]);
            $('#credit-note', myModal).val(columnValues[22]);
            $('#credit-narration', myModal).val(columnValues[23]);
    ...
    $('#btn-update-credit-modal').click(function (event) {
        ...
        debugger;
        if (IsValidCreditTableModal()) {
            creditTable.row($(this).attr('rowindex')).data([
                                 checkbox,
                                 PersonId,
                                 PersonText,
                                 businessOfficeId,
    ...
    $('#tbl-credit tbody').click("input[type=checkbox]", function () {
        ...
        $('#tbl-credit input[type="checkbox"]:checked').each(function (index) {
            ...
            if (isChecked) {
                ...
                //$('#btn-delete-' + 'credit' + '-dt').removeAttr("disabled");
                //$('#btn-edit-' + 'credit' + '-dt').removeAttr("disabled");
                //$('#btn-add-' + 'credit' + '-dt').prop('disabled', true);
                //$('#btn-delete-' + 'credit' + '-dt').prop('disabled', false);
                //$('#btn-edit-' + 'credit' + '-dt').prop('disabled', false);
    ...
    function credit(_dataTableName) {
        ...
        $('#tbl-credit TBODY TR').each(function () {
            ...
            if (typeof columnValue == 'undefined' && columnValue == null) {
                ...
                $.each(arr1, function (i, val) {
                    if (!isNaN(parseFloat(val))) {
                        other += parseFloat(val) ? parseFloat(val) : 0;
                    }
                })


                let total = principleamount + other;
                creditTable.cell(currentRow, 9).data(principleamount ? principleamount : 0);
                creditTable.cell(currentRow, 14).data(other ? other : 0);
                creditTable.cell(currentRow, 15).data((total + parseFloat(columnValue[13])) || 0);
                principleamountarry.push(columnValue[15]);
            ...
            //alert(Total5);
            // var totalElements = creditTable.column(15).data().toArray();
            ///alert(totalElements)
        })
        $("#tbl-credit").on('search.dt', function () {
            debugger
            var t = creditTable.column(15, { page: 'current' }).data().sum();
            $('#credit-amount').attr('data-id', t.toFixed(2));
            $('#credit-amount').html(t.toFixed(2));
            $('#debit-amount').attr('data-id', t.toFixed(2));
            $('#debit-amount').html(t.toFixed(2));
            numToWords2(t, 'credit');
            numToWords2(t, 'debit');
        });
        $('#tbl-debit > tbody > tr').each(function (index) {
            debugger;
    ...
    function IsValidCreditTableModal() {
        ...
        //id = $('#credit-businessOffice-id option:selected').val();
        ...
        admissionfeesformembership = parseInt($('#credit-admission-fees-for-member-ship').val()) ? parseInt($('#credit-admission-fees-for-member-ship').val()) : 0;
        ...
        mintransaction = parseFloat($('#credit-transaction-amount').attr('min'));
        maxtransaction = parseFloat($('#credit-transaction-amount').attr('max'));
        ...
        if (min >= numberofshares) {
            ...
            $('#credit-number-of-shares').after('<div class="error" style="color:red">Please enter a value greater than to ' + min + ' </div>');
            return false;
        ...
        if ((PersonId == "0" || generalLedgerId.trim().length < 36) || (customerAccountId.trim().length < 36) || (startCertificateNumber == "") || (endCertificateNumber == "") || (narration == "") || (mintransaction >= transactionAmount) || (maxtransaction <= transactionAmount)) {
            ClearCreditDivErrors();
            PersonId = "0";
            if (PersonId == "0")
                $('#credit-person').after('<div class="error" style="color:red">Please Select Customer Name </div>');
            ...
            if ((mintransaction >= transactionAmount) || (maxtransaction <= transactionAmount))
                $('#credit-transaction-amount').after('<div class="error" style="color:red">Please enter a value greater than or less than or equal to ' + mintransaction + ' and  ' + maxtransaction + ' </div>');
    ...
    function debit(_dataTableName) {
        ...
        $('#tbl-debit TBODY TR').each(function () {
            ...
            if (typeof columnValue == 'undefined' && columnValue == null) {
                ...
                debitTable.cell(currentRow, 9).data(total ? total : 0);
        ...
        $('#tbl-credit > tbody > tr').each(function (index) {
            ...
            if (typeof columnValues != 'undefined' && columnValues != null) {
                debugger;
                const credittotal = principleamountarry.reduce((a, b) =>  parseFloat(a) + parseFloat(b), 0);
                creditTable.cell(0, 15).data(credittotal);
                $('td', currentRow).eq(0).find(".checks").attr('disabled', true);
                $('th', currentRow).find("#select-all-debit").attr('disabled', true);
    ...
    function IsValidDebitModal() {
        ...
        if ((generalLedgerId.length < 36) || (customerAccountId.length < 36) || (narration == "") || ($('input[id^=CessionReason-]:checked').length == 0)) {
            ClearDebitDivErrors();
            if (generalLedgerId.trim().length < 36)
            ...
            if (CessionReason == "None")
                $('.age-proof-submission').after('<div class="error" style="color:red">You must select an Cession Reason </div>');
        ...
        if (transactiontype === '408d891d-244a-4b8f-b590-0e1b2a6d5afb') {
            ...
            $('#tbl-credit > tbody > tr').each(function (index) {
                ...
                if (typeof columnValues != 'undefined' && columnValues != null) {
                    ...
                    creditTable.cell(0, 15).data(total);
                    $('td', currentRow).eq(0).find(".checks").attr('disabled', true);
                    ...
                    creditTable.cell(0, 14).data("nane");
                    //creditTable.cell(0, 15).data(total);
    ...
    function ClearCreditModalInputs() {
        ...
        $('.amount-badge-text').empty();
        $('.amount-badge-text1').empty();
        $('#admissionfeesformembership').addClass('d-none');
    ...
    function ClearDebitDivErrors() {
        $('#debit-person').next("div.error").remove();
        $('#debit-general-ledger-id').next("div.error").remove();
        $('#debit-customer-account-id').next("div.error").remove();
        $('#debit-transaction-amount').next("div.error").remove();
        $('#debit-start-certificate-number').next("div.error").remove();
        $('#debit-end-certificate-number').next("div.error").remove();
        $('#debit-number-of-shares').next('div.error').remove();
        $('#debit-narration').next("div.error").remove();
        $('#age-proof-submission').next("div.error").remove();
    }
    ...
    $('#transaction-type-id').change(function () {
        ...
        if (transactiontype === "") {
            ...
            $('#btn-add-debit-dt').addClass('read-only');
            $('.cash-denomination').addClass('d-none');
            creditTable.clear().draw();
            return false;
        }
        if (transactiontype === '42882154-c991-468f-a645-59eb12939b1d') {
            model += "#credit-modal";
            jQuery('#total-amount').attr('data-id', '0');
            $('#btn-add-credit-dt').removeClass('read-only');
            $('#btn-add-debit-dt').addClass('read-only');
            $('#btn-edit-debit-dt').addClass('read-only');
            $('#btn-delete-debit-dt').addClass('read-only');
            ...
            $('#btn-edit-denomination-dt').hide();
            $('#btn-add-credit-dt').addClass('read-only');
            $('#btn-add-debit-dt').removeClass('read-only');
            $('#btn-edit-debit-dt').addClass('read-only');
            $('#btn-delete-debit-dt').addClass('read-only');
            jQuery('#total-amount').attr('data-id', '0');
    ...
    $(document).on("change", "input[id^=credit-]", function () {
        ...
        if (arr.length === 0 || arr[0] === 0) {
            ...
            txt += "Being " + array[4] + " Shares Sold @ Face Value RS " + array[3].trim() + " To Mr. " + personText1[0].trim() + " And Receiving Rs " + admissionfee.trim() + " " + stationaryfee.trim() + " " + buildingfund.trim() + " " + reservefund.trim() + "";
        ...
        let admissionfeesformembership = parseFloat($('#credit-admission-fees-for-member-ship').val());
        //let balance = parseFloat($('#badge-amount').data("amount"));
        let sharesholdinglimit = parseFloat($('#shares-holding-limit').attr('data-sharesLimit'));
        let previouscertificatenumber = parseFloat($('#previous-certificate-number').attr('data-previousCertificateNumber'));
        let totals = parseFloat(sharesfacevalue * numberofshares);
    ...
    $('#btn-add-denomination-modal').click(function (event) {
        ...
        if (IsValiddenominationModal('credit', 'debit')) {
            ...
            if (confirm('Are You Sure To Delete This Record?')) {
                if ($("#tbl-denomination tbody input[type='checkbox']:checked").each(function () {
                    denominationTable.row($("#tbl-denomination tbody input[type='checkbox']:checked").parents('tr').get(0)).remove().draw();
                      ...
                      $('#tbl-denomination> tbody > tr').each(function (index) {
                     let total = parseInt($('#denomination-amount').attr('data-id'));
                                        let currentRow = $(this).closest('tr');
                                        let columnValue = (denominationTable.row(currentRow).data());
                                        if (typeof columnValue != 'undefined' && columnValue != null) {
                                        if (total > 0) {
                                        denominationTable.cell(currentRow, 4).data(columnValue[4]);
                                        $('#denomination-amount').attr('data-id', total.toFixed(2));
                                        $('#denomination-amount').html(total.toFixed(2));
                                        numToWords3(columnValue[4]);
                }
                else {
                                           denominationTable.row($(this).closest("tr").get(0)).remove().draw();
                }
                }
                })
                      Denomination('denomination');
    ...
    function Denomination(_dataTableName) {
        let denominationtotalarray = [];
        $('#tbl-denomination> tbody > tr').each(function (index) {
            let currentRow = $(this).closest('tr');
            let columnValue = (denominationTable.row(currentRow).data());
            let denominationtotal = 0;
            denominationtotal += (Math.round(columnValue[2] * columnValue[3]));
            denominationtotalarray.push(denominationtotal);
            denominationTable.cell(currentRow, 4).data(denominationtotal);
        })
        const result = denominationtotalarray.reduce((a, b) =>  parseFloat(a) + parseFloat(b), 0);
    ...
    function IsValiddenominationModal(table) {
        debugger;
    ...
    var ExcelToJSON = function () {
        ...
        var fileExtension = ['xlsx', 'xls', 'xml'];
        var t = name;
        this.parseExcel = function (file) {

            if (($(".upload").val().toLowerCase().split('.')[1] == 'xlsx' | 'xls' | 'xml')) {
                ///if (regex.test(fileUpload.value.toLowerCase())) {
                //var regex = new RegExp("([a-zA-Z0-9\s_\\.\-:])+(" + fileExtension.join('|') + ")$");
                //var y=($("#upload").val().toLowerCase().split('.')[1] );
                ...
                reader.onload = function (e) {
                    ...
                    workbook.SheetNames.forEach(function (sheetName) {
                        ...
                        var dataList = (JSON.parse(JSON.stringify(XL_row_object)));
                        var columns = [];
                        var my_columns = [];
                        let row = null;
                        let transactiontype = $('#transaction-type-id option:selected').val();
                        ...
                        for (var i = 0; i < dataList.length; i++) {
                            var columns = Object.values(dataList[i])
                            row = table.row.add([
